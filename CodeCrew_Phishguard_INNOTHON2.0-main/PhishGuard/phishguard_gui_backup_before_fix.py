...
class PhishGuardGUI(QMainWindow):
    ...
    def createDashboardTab(self, parent_widget):
        ...
        tables_splitter.setSizes([600, 400])  # Set relative sizes (threats table larger)        # Add widgets to dashboard layout
    ...
    def createStatWidget(self, parent_layout, title, value, color, icon_name=None):
        ...
        stat_frame.setStyleSheet(f"background-color: white; border-radius: 8px; border-top: 4px solid {color}; border: 1px solid #e0e0e0;")
    ...
    def createThreatsTab(self, parent_widget):
        ...
        actions_layout.addWidget(export_button)
          # Add threats table to main layout
        threats_layout.addWidget(threats_table_frame)        # Populate with sample data
        self.populateThreatsList()
    
    def createReportsTab(self, parent_widget):
        """Create the reports tab with improved form visibility."""
    ...
    # Report generation section - FIXED VISIBILITY
        report_frame = QFrame()
        report_frame.setStyleSheet(
            "QFrame { background-color: white; border-radius: 8px; padding: 20px; border: 2px solid #0066cc; }"
        )
        report_frame.setFixedHeight(520)  # Increased fixed height to ensure all elements are visible
    ...
    # Create a container widget for the form
        ...
        form_container.setMinimumHeight(280)  # Ensure minimum height for form container
    ...
    # Form layout for report options
        ...
        form_layout.setVerticalSpacing(12)  # Reduced vertical spacing to fit all elements
        ...
        self.report_type_combo.setFixedHeight(35)  # Fixed height
        ...
        self.time_period_combo.setFixedHeight(35)  # Fixed height
        ...
        self.dept_filter.setFixedHeight(35)  # Fixed height
        ...
        # Format combo with enhanced visibility        format_label = QLabel("Format:")
        ...
        self.format_combo.setFixedHeight(32)  # Slightly reduced height
    ...
    # Buttons layout
        ...
        buttons_layout.setContentsMargins(0, 15, 0, 0)  # Reduced top margin
        ...
        preview_button.setMinimumHeight(36)  # Reduced height
        ...
        generate_button.setMinimumHeight(36)  # Reduced height
    ...
    def generateReport(self):
        ...
        try:
            ...
            filename_base = f"{report_type.replace(' ', '_')}_{department.replace(' ', '_')}_{timestamp}"
            ...
            extension = extensions.get(format_type, ".pdf")
            
            # In a real implementation, this would use the ReportGenerator to create the actual report
            # Default save path (can be changed by user in the save dialog)
            default_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "reports")
            os.makedirs(default_path, exist_ok=True)
            
            # For each format, define the subdirectory
            format_dirs = {
                "PDF": "pdf",
                "HTML": "html",
                "CSV": "csv",
            ...
            format_dir = format_dirs.get(format_type, "")
            if format_dir:
                default_path = os.path.join(default_path, format_dir)
                os.makedirs(default_path, exist_ok=True)
            
            default_file = os.path.join(default_path, filename_base + extension)
            
            # Open a file save dialog for the user to choose where to save the report
            ...
            if not file_path:
                ...
                self.statusBar.showMessage("Report generation canceled", 5000)
                return
            
            # Create a simulated file for demonstration purposes
            # In a real implementation, this would generate the actual report
            try:
                if format_type == "HTML":
                    self.createDemoHtmlReport(file_path, report_type, time_period, department)
                elif format_type == "CSV":
                    self.createDemoCsvReport(file_path, report_type, time_period, department)
                elif format_type == "Excel":
                    self.createDemoExcelReport(file_path, report_type, time_period, department)
                else:  # Default to PDF
                    self.createDemoPdfReport(file_path, report_type, time_period, department)
                
                # Show success message with option to open the file
                msg_box = QMessageBox(self)
                msg_box.setWindowTitle("Report Generated")
                msg_box.setText(f"The {report_type} has been generated in {format_type} format and saved to:\n{file_path}")
                msg_box.setIcon(QMessageBox.Information)
                
                # Add buttons to open file or view containing folder
                ...
                self.statusBar.showMessage(f"Report generated successfully and saved to {file_path}", 5000)
                
            except Exception as e:
                logger.error(f"Error saving report file: {str(e)}")
                QMessageBox.warning(self, "Save Error", f"Could not save report file: {str(e)}")
                self.statusBar.showMessage("Error saving report file", 5000)
    ...
    def createDemoHtmlReport(self, file_path, report_type, time_period, department):
        """Create a demo HTML report for demonstration purposes."""
...
<head>
    ...
    <title>{report_type} - PhishGuard</title>
    <style>
        body {{
...
<body>
    <div class="container">
        <div class="header">
            <h1>{report_type}</h1>
            <p>Time Period: {time_period} | Department: {department}</p>
            <p>Generated on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
        </div>
        
        <h2>Report Content</h2>
        <p>This is a demo report. In a real implementation, this would contain actual data.</p>
...
"""
    ...
    def createDemoCsvReport(self, file_path, report_type, time_period, department):
        """Create a demo CSV report for demonstration purposes."""
        import csv
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            
            # Write header row with metadata
            writer.writerow([f"{report_type} - PhishGuard"])
            writer.writerow([f"Time Period: {time_period}"])
            writer.writerow([f"Department: {department}"])
            writer.writerow([f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"])
            writer.writerow([])  # Empty row for spacing
            
            # Write data headers
            if "Threat Summary" in report_type:
                writer.writerow(["Date", "Sender", "Subject", "Risk Score", "Threat Type"])
                # Sample data
                writer.writerow(["2025-05-10", "suspicious@example.com", "Urgent: Your account has been compromised", "85%", "Phishing"])
                writer.writerow(["2025-05-10", "newsletter@unknown-domain.com", "Important Document Attached", "65%", "Suspicious Attachment"])
                writer.writerow(["2025-05-09", "info@company-portal.net", "Update your password now", "40%", "Social Engineering"])
            elif "Department Risk" in report_type:
                writer.writerow(["Department", "Threat Count", "Avg Risk Score", "Most Common Threat"])
                # Sample data
                writer.writerow(["Finance", "12", "78%", "Phishing"])
                writer.writerow(["Human Resources", "8", "62%", "Social Engineering"])
                writer.writerow(["IT", "4", "45%", "Malware"])
            else:
                # Generic headers for other report types                writer.writerow(["Category", "Value", "Details"])
                writer.writerow(["Sample 1", "Value 1", "Detail info"])
                writer.writerow(["Sample 2", "Value 2", "Detail info"])
    
    def createDemoExcelReport(self, file_path, report_type, time_period, department):
        """Create a demo Excel report for demonstration purposes."""
        import openpyxl
        ...
        info_font = Font(name='Calibri', italic=True, size=10)
        ...
        # Add title
        worksheet['A1'] = report_type
        worksheet['A1'].font = title_font
        
        # Add metadata
        worksheet['A2'] = f"Time Period: {time_period}"
        worksheet['A2'].font = info_font
        worksheet['A3'] = f"Department: {department}"
        worksheet['A3'].font = info_font
        worksheet['A4'] = f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        worksheet['A4'].font = info_font
        ...
        if "Threat Summary" in report_type:
            # Add headers
            headers = ["Date", "Sender", "Subject", "Risk Score", "Threat Type"]
            ...
            for row_num, row_data in enumerate(data, start_row + 1):
                for col_num, cell_value in enumerate(row_data, 1):
                    ...
                    cell.border = border
                    if col_num == 4:  # Risk Score column
                        cell.alignment = center_align
                    
        elif "Department Risk" in report_type:
            # Add headers
            headers = ["Department", "Threat Count", "Avg Risk Score", "Most Common Threat"]
            for col_num, header in enumerate(headers, 1):
                cell = worksheet.cell(row=start_row, column=col_num, value=header)
                cell.font = header_font
                cell.fill = header_fill
            ...
            for row_num, row_data in enumerate(data, start_row + 1):
                for col_num, cell_value in enumerate(row_data, 1):
                    ...
                    cell.font = normal_font
                    cell.border = border
                    if col_num in (2, 3):  # Numeric columns
                        cell.alignment = center_align
        
        else:
            # Generic content
            worksheet.cell(row=start_row, column=1, value="Report Content")
            worksheet.cell(row=start_row, column=1).font = title_font
            worksheet.cell(row=start_row+1, column=1, value="This is a demo report. In a real implementation, this would contain actual data.")
            worksheet.cell(row=start_row+1, column=1).font = normal_font
        
        # Add footer
        footer_row = start_row + 10
        worksheet.cell(row=footer_row, column=1, value="This report was automatically generated by PhishGuard.")
        worksheet.cell(row=footer_row, column=1).font = info_font
        worksheet.cell(row=footer_row+1, column=1, value="If you have questions about this report, please contact your IT department.")
        worksheet.cell(row=footer_row+1, column=1).font = info_font
    ...
    def createDemoPdfReport(self, file_path, report_type, time_period, department):
        """Create a demo PDF report for demonstration purposes."""
        ...
        # Add title
        content.append(Paragraph(report_type, title_style))
        content.append(Spacer(1, 0.25*inch))
        
        # Add report info
        content.append(Paragraph(f"Time Period: {time_period}", info_style))
        content.append(Paragraph(f"Department: {department}", info_style))
        content.append(Paragraph(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", info_style))
        content.append(Spacer(1, 0.5*inch))
        
        # Add report content based on type
        if "Threat Summary" in report_type:
            content.append(Paragraph("Threat Summary", heading_style))
            content.append(Spacer(1, 0.25*inch))
            content.append(Paragraph("Recent threats detected in your organization", normal_style))
            content.append(Spacer(1, 0.25*inch))
            
            # Create table data
            data = [
                ["Date", "Sender", "Subject", "Risk Score", "Threat Type"],
                ["2025-05-10", "suspicious@example.com", "Urgent: Your account has been compromised", "85%", "Phishing"],
                ["2025-05-10", "newsletter@unknown-domain.com", "Important Document Attached", "65%", "Suspicious Attachment"],
                ["2025-05-09", "info@company-portal.net", "Update your password now", "40%", "Social Engineering"]
            ]
            ...
            content.append(Paragraph("Department Risk Analysis", heading_style))
            content.append(Spacer(1, 0.25*inch))
            content.append(Paragraph("Risk assessment by department", normal_style))
            content.append(Spacer(1, 0.25*inch))
            
            # Create table data
            data = [
                ["Department", "Threat Count", "Avg Risk Score", "Most Common Threat"],
                ["Finance", "12", "78%", "Phishing"],
                ["Human Resources", "8", "62%", "Social Engineering"],
                ["IT", "4", "45%", "Malware"],
                ["Marketing", "7", "58%", "Suspicious Links"],
            ...
            content.append(Spacer(1, 0.25*inch))
            content.append(Paragraph("This is a demo report. In a real implementation, this would contain actual data based on the selected report type.", normal_style))
        
        content.append(Spacer(1, 0.5*inch))
        
        # Add footer
        content.append(Paragraph("This report was automatically generated by PhishGuard.", info_style))
        content.append(Paragraph("If you have questions about this report, please contact your IT department.", info_style))
